...
def get_node(successor_xy_name, successor_current_time, nodes_dict, open_nodes, closed_nodes, goal, **kwargs):

    # curr_h = h_func(node_current, goal)
    # succ_h = h_func(nodes_dict[successor_xy_name], goal)
    # new_t = node_current.t + abs(curr_h - succ_h)
    # new_t = node_current.t + 1

    new_ID = f'{successor_xy_name}'
    ...
    new_node = Node(x=node.x, y=node.y, t=successor_current_time, neighbours=node.neighbours)
    new_node.ID = new_node.xy_name
    return new_node, 'new'


def reset_nodes(start, goal, nodes, **kwargs):
    for node in nodes:
        node.ID = node.xy_name
        node.parent = None
        node.g = 0
        node.t = 0
        node.h = 0
...
def a_star_xy(start, goal, nodes, h_func, plotter=None, middle_plot=False, iter_limit=1e100, nodes_dict=None, **kwargs):
    """
    ...
    while len(open_nodes) > 0:
        ...
        for successor_xy_name in node_current.neighbours:
            ...
            curr_h = h_func(node_current, goal)
            succ_h = h_func(nodes_dict[successor_xy_name], goal)
            successor_current_time = node_current.t + abs(curr_h - succ_h)

            node_successor, node_successor_status = get_node(
                successor_xy_name, successor_current_time, nodes_dict, open_nodes, closed_nodes, goal, **kwargs
            )
...
def main():
    ...
    result = a_star_xy(start=node_start, goal=node_goal, nodes=nodes, h_func=dist_heuristic, plotter=plotter,
                    middle_plot=True)
...
def try_a_map_from_pic(**kwargs):
    ...
    path, info = a_star_xy(start=node_start, goal=node_goal, nodes=nodes, h_func=h_func,
                          plotter=plotter, middle_plot=True, nodes_dict=nodes_dict, **kwargs)
    profiler.disable()
    if path:
        print('The result is:', *[node.xy_name for node in path], sep='->')
        print('The result is:', *[node.ID for node in path], sep='->')
    # ------------------------- #
    # ------------------------- #
    plt.show()
    # plt.close()
